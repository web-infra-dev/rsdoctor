...
3. Determine the PR target branch.
   Use `main` by default because it is the v2 development branch.
   Use `v1.x` only for v1 releases, v1 backports, or changes that only apply to v1.
   Changes should be merged to `main` first whenever possible, then backported to `v1.x` if they also need to ship in v1.
   If a PR targets `v1.x` directly, explain in the PR body why it does not apply to `main`.
   If the correct target branch is ambiguous, ask the user to confirm before creating the PR.
...
8. Push the branch only after re-checking the branch name. Never push `main` or `v1.x` directly.

9. Create the PR with `gh pr create --base <target-branch>`.
