...
在开发过程中遇到构建报错或需要进行构建优化时，往往很难抓取构建过程中的行为或拆解产物的关系。此时，你可以通过 **Rsdoctor** 直观地看到项目构建器的编译行为及详细构建信息，从而更深入地分析项目问题，比如：
- 更加直观地看到重复包的上游依赖链路。
- 深入分析 Loader 的调用过程与构建耗时，包括 Rspack 的 builtin:swc-loader。
- 更清晰地查看构建产物中资源与模块的关系，以及资源内打包的模块代码。
- 直观地看到 Resolver 解析行为及解析耗时。
- ... ...
