...
export function filterLoaders(loaderData: SDK.ResourceLoaderData, keyword: string, loaderNames: string[]) {
  return loaderData.loaders.filter((item) => {
    return filterLoader(loaderData.resource.path, item.loader, keyword, loaderNames);
  });
}

export function filterLoader(resourcePath: string, loader: string, keyword: string, loaderNames: string[]) {
  if (keyword) {
