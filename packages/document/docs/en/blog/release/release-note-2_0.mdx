...
Compiler selection is also available in Agent CLI. JSON reports now include an index of compiler names and data files. For applications that build both client and server bundles, `compilers list` discovers the available compilers and `--compiler` selects the data to analyze:
...
Each query reads one compiler's data, keeping client and server results separate. Reports with a single indexed compiler are selected automatically; reports with multiple compilers require an explicit name. See [Selecting a compiler](/guide/start/ai#selecting-a-compiler) for details.
