...
- Treemap adds full-screen mode, a collapsible sidebar, breadcrumb navigation, and click-to-zoom. You can drill into deeply nested modules while keeping the current hierarchy visible.
- Asset and chunk filters, regular-expression module search, result highlighting, and direct access to module details shorten the path from spotting an unexpected size to identifying the responsible code.
- Stat, Parsed, and Gzipped size modes now stay consistent across the Treemap, sidebar, and tooltips. JavaScript asset filtering removes unrelated resources from module-focused views, while responsive layouts and expandable cards make better use of the available space.
...
[![Contributors](https://contrib.rocks/image?repo=web-infra-dev/rsdoctor)](https://github.com/web-infra-dev/rsdoctor/graphs/contributors)
