# mode

:::warning

The top-level `mode` option was removed in Rsdoctor 2.x. Use [output.mode](/config/options/output#mode) instead. See the [mode configuration migration guide](/guide/migration/migration-v2#mode).

:::

The removed option accepted the following values:

- **Type:** `"normal" | "brief" | "lite"`
  - `lite` was removed and is ignored in Rsdoctor 2.x. Use [output.reportCodeType](/config/options/output#reportcodetype) instead.
- **Optional:** `true`
- **Default:** `normal`

These values selected the following report behaviors:

import ModeIntro from '@en/shared/mode-intro.md';

<ModeIntro />
