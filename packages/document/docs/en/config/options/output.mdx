...
- **brief:** Generates `rsdoctor-report.html` in the build output directory. All build analysis data is embedded in this standalone file, which can be opened directly in a browser. See [mode: 'brief'](#mode-brief) for additional options.

:::tip
`output.mode` does not accept `lite`. The legacy `mode: 'lite'` option is deprecated in Rsdoctor 2.x and retained for backward compatibility. Use [`output.reportCodeType`](#reportcodetype) instead.
:::
...
:::warning
The legacy `output.compressData` option is deprecated in Rsdoctor 2.x and retained for backward compatibility. Use `output.mode: 'brief'` and `output.options.type: ['json']` instead. See [compressData configuration migration](/config/options/options-v2#outputcompressdata).
:::
