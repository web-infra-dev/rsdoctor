...
<Badge text="V1.2.4 New API" type="tip" />
...
> This mode does not have a lite configuration option. The lite mode will be deprecated. Refer to [lite mode deprecation notice](./options-v2.mdx#lite-mode-deprecation-notice).
...
- **Description:**
  ...
  Data output order from rich to simple: `noModuleSource` > `noAssetsAndModuleSource` > `noCode`.
