...
When we **only need** to use Rsdoctor's [Bundle Size](../usage/bundle-size) bundle size analysis feature, we can configure the corresponding [Features](/config/options/options) option when integrating the Rsdoctor plugin. Refer to the code snippet below:
...
Therefore, the loader timing provided by Rsdoctor is an **estimated** value. The timing data we provide is adapted to handle scenario 1 and scenario 2 from the diagram. As for scenario 3, we are still exploring solutions.
...
You can add the `lite` field to the `features` array to use the lite mode. Note: Since the `features` array overrides the default configuration when it is an array, you need to manually add the required features:

- If you need build-time analysis, add `loader` and `plugins` to the `features` array to enable timing analysis for `loader` and `plugins`.
- If you need bundle analysis, add `bundle` to the `features` array to enable bundle analysis.

The following example enables the lite mode along with build-time and bundle analysis:
...
- **Cause**: During the build process, source code information is stored, which exceeds the memory limit. Enabling the `lite` mode can alleviate this issue.
- **Difference**: The difference between the `lite` mode and the normal mode is that the `lite` mode no longer stores **source code information**, only **bundled code** is stored. The code displayed in the analysis report will also **only show bundled code**.
