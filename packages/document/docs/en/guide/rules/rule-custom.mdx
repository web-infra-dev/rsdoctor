...
When writing custom rules, install `@rsdoctor/core` and use the defineRule function from `@rsdoctor/core` to define unified Rsdoctor rules.
...
Rsdoctor displays custom rule results in a warning list. For more details, see [Display Components](#display-components).
...
- Component Code [Code](https://github.com/web-infra-dev/rsdoctor/blob/main/packages/client/src/components/Alerts/list.tsx)
...
This data type is still used by core rules. The current client displays these results in the warning list without a dedicated code-view component.
...
When performing rule detection and analysis, it is common to perform AST analysis on modules and other operations. To provide more auxiliary functions, we also provide `@rsdoctor/core`, which contains many useful utility functions and methods.
