...
When writing custom rules, install `@rsdoctor/core` and use the defineRule function from `@rsdoctor/core/rules` to define unified Rsdoctor rules.
...
- Component Code: [Code](https://github.com/web-infra-dev/rsdoctor/blob/main/packages/client/src/components/Alert/view.tsx)
...
When performing rule detection and analysis, it is common to perform AST analysis on modules and other operations. To provide more auxiliary functions, we also provide `@rsdoctor/core/rule-utils`, which contains many useful utility functions and methods.
