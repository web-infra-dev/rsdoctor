...
This rule detects **modules that are bundled solely due to side effects**. This is often caused by unintended tree-shaking failures — for example, a missing or incorrect `"sideEffects"` field in `package.json`, or non-tree-shakeable import patterns — resulting in the entire module being bundled even though none of its exports are used.

#### Common causes

- The package's `package.json` is missing `"sideEffects": false` (or is incorrectly set to `true`), preventing the bundler from pruning unused exports.
- An import statement like `import 'some-module'` or `import './styles.css'` is treated as a side-effect-only import, but the intended use was to consume exports.
- Barrel files (index files that re-export many things) cause the whole module to be kept when only a side-effect import is present.
- Using **import-then-export** patterns in barrel files (see below) prevents the bundler from statically analyzing which exports are used.

#### Re-export patterns in barrel files

When writing a barrel file (e.g. `index.ts`) that re-exports from multiple modules, the export pattern you choose has a significant impact on tree-shaking:
...
**Method 1** (`export { ... } from`) and **Method 2** (`export * from`) are both static re-exports. The bundler can statically analyze which exports are used and tree-shake the rest.

**Method 3** (import then export) introduces an intermediate binding: each `import` statement causes the module to be evaluated (i.e. treated as having side effects), so the bundler cannot safely drop the module even when none of its exports are consumed. Avoid this pattern in barrel files.
...
3. **Avoid unintended side-effect imports**: Convert `import 'module'` patterns to explicit `import { foo } from 'module'` patterns where the exports are needed.
4. **Use static re-exports in barrel files**: Replace `import` + `export` patterns with `export { ... } from` or `export * from` to keep re-exports statically analyzable.

#### Configuration

- **ignore**: Module path patterns to ignore (string match: if the module path contains any of these strings, it is ignored).
- **include**: When a module is under `node_modules`, only check modules whose path contains one of these strings. By default, modules under `node_modules` are not checked.
...
interface Config {
  ...
  ignore: string[];
  /**
   * When a module is under node_modules, only check modules whose path contains these strings.
