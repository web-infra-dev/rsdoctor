...
- `on` indicates when the workflow runs, commonly set to `pull_request` and `push`, and also supports `workflow_dispatch` for manual triggering.
  ...
  - On `workflow_dispatch` (manual trigger), it combines the behavior of `push` and `pull_request`: it uploads artifacts, and if `dispatch_target_branch` is specified, it also performs baseline comparison analysis.
