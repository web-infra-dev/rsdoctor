...
- Install it in your project (optional, because the skill can run the CLI with `npx` automatically):

<PackageManagerTabs command="add @rsdoctor/agent-cli -g" />
...
- `rsdoctor-agent list`: List currently available tool names, so you can confirm which `<tool-name>` values can be used with `query`.
- `rsdoctor-agent query <tool-name> --data-file <path> [--input <json>]`: Invoke a specified tool and return the result.
...
- Skill execution example:
  ...
  As shown in the animation above, the AI tool runs the corresponding `@rsdoctor/agent-cli` commands based on the Rsdoctor skill, retrieves the required data, and outputs optimization suggestions after consolidating the analysis. It also provides a deeper follow-up analysis plan. You can continue with `Next Step` to further investigate project build issues.
