...
A project may build client, server, or worker bundles with separate compilers. Each build has its own output and dependencies, so analysis needs a clear target to avoid treating server dependencies as client bundle size problems.

Agent CLI can list the compilers in a report and select one for analysis. It automatically selects a compiler when the report's index contains only one; with multiple compilers, the target must be specified. Each query reads only the selected compiler's data, keeping results from different builds separate.

When using a coding agent, include the target in your question, for example: “Analyze the client build and find duplicate dependencies that could reduce bundle size.” You do not need to remember the commands. For manual queries, see the [`@rsdoctor/agent-cli` README](https://github.com/web-infra-dev/rsdoctor/tree/main/packages/agent-cli#selecting-a-compiler).

```bash
# List available compilers
rsdoctor-agent compilers list --data-file ./rsdoctor-data.json
# Analyze the server compiler
