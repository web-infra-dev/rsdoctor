...
**Rsdoctor** provides the `Bundle Size` module for analyzing build artifacts, including **resource sizes**, **duplicate packages**, and **module relationships**:
...
- **`Assets`**: Resources are files that ultimately exist in the output folder, including JavaScript, CSS, images, fonts, media, and other file types. Each Chunk has corresponding [Assets resources](https://rspack.rs/misc/glossary#asset).
- **`Module`**: Multiple Modules combine to form a Chunk. For more information about Module types, please refer to [Rspack Modules](https://rspack.rs/misc/glossary#module-type).
...
- **`Initial Chunk`**: **initial (initialization)** is the main Chunk of the entry point. This Chunk contains all the modules specified by the entry point and their dependencies, unlike the **Chunk** resources for "**on-demand loading**".
  - For more information about Initial Chunk, please refer to [Initial Chunk Introduction](https://rspack.rs/misc/glossary#initial-chunk).
...
- **`Concatenated Module`**: A concatenated module is a technique that combines multiple modules into one closure during packaging. Wrapping each module in a separate function adds runtime overhead in the browser. Optimization can be achieved by enabling the [`optimization.concatenateModules`](https://rspack.rs/misc/glossary#scope-hoisting) parameter.
...
The **Assets** tag is shown in the left image, from left to right representing: **Resource Size**, **[Initial Chunk](https://rspack.rs/misc/glossary#initial-chunk)**, and **Code View**.
...
Rspack provides [BannerPlugin](https://www.rspack.rs/plugins/webpack/banner-plugin#bannerplugin), a built-in plugin for adding content to the beginning or end of generated chunks.
...
1. The project uses BannerPlugin in `rspack.config.(js|ts)`.
