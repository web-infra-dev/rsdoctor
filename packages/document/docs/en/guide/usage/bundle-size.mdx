...
First, **[Concatenated Module](https://webpack.js.org/plugins/module-concatenation-plugin/#root)** refers to multiple modules that are merged into a single closure, which cannot be decomposed through AST syntax analysis. However, concatenated modules may contain code from different packages, making the analysis of sub-modules within concatenated modules a key focus, especially for projects using the ["all-in-one"](https://rsbuild.rs/guide/optimization/code-splitting#all-in-one) bundling approach.

Rsdoctor supports the decomposition of **[Concatenated Module](https://webpack.js.org/plugins/module-concatenation-plugin/#root)** and accurately calculates the true bundled size of sub-modules within concatenated modules, helping developers accurately identify the actual build size after Tree Shaking, analyze the impact of merged modules on final bundle size, and optimize code splitting strategies.
...
> As shown in the figure below, this is the analysis of 'all-in-one' bundling. The left image shows the previous inability to analyze, while the right image shows the analyzable situation.
