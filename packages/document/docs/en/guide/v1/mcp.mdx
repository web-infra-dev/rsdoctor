# Rsdoctor 1.x MCP server

:::warning
`@rsdoctor/mcp-server` is no longer maintained. For Rsdoctor 2.x AI-assisted build analysis workflows, use [`@rsdoctor/agent-cli`](/guide/start/ai#agent-cli) instead.

The instructions below are archived for Rsdoctor 1.x users and use the legacy `@rsdoctor/rspack-plugin` package. They do not apply to Rsdoctor 2.0.
:::
...
:::warning
This archived workflow requires the following Rsdoctor 1.x plugin version:

- @rsdoctor/rspack-plugin >= 1.1.2

For Rsdoctor 2.0, use `@rsdoctor/agent-cli` instead.
:::

### 1. Plugin configuration

Install the archived 1.x Rspack plugin, then add `RsdoctorRspackPlugin` to your Rspack configuration:

```bash
pnpm add -D @rsdoctor/rspack-plugin@1
```
...
<div style={{ display: 'inline-block', marginTop: 16 }}>
  <a href="cursor://anysphere.cursor-deeplink/mcp/install?name=rsdoctor&config=eyJjb21tYW5kIjoibnB4IC15IEByc2RvY3Rvci9tY3Atc2VydmVyQDEifQ">
    <img
...
<div style={{ display: 'inline-block', marginTop: 16 }}>
  <a href="vscode:mcp/install?%7B%22name%22%3A%22rsdoctor%22%2C%22type%22%3A%22stdio%22%2C%22command%22%3A%22npx%22%2C%22args%22%3A%5B%22-y%22%2C%22%40rsdoctor%2Fmcp-server%401%22%5D%7D">
    <img
...
```bash
npx @rsdoctor/mcp-server@1 --compiler web
```
...
```bash
npx @rsdoctor/mcp-server@1 --port 1000
```
...
- Check if the Rsdoctor plugin version meets the requirements. [Version requirements](#-version-requirements)
