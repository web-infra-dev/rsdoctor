...
通过在 Rspack 中集成 Rsdoctor 的原生插件，我们将 Rsdoctor 中耗时较大的数据处理逻辑下沉到 Rspack 的构建阶段。这一优化显著提升了 Rspack 中 Rsdoctor 的构建分析效率，减少了 **17%** 的分析时间，项目的模块数量越大，提升效率越显著，[详情](/config/options/options#enablenativeplugin)。
