...
<img src="/loader-timeline-overall.png" />
...
<img style={{ width: 700, margin: 'auto' }} src="/module-analysis-tree.png" />
...
通过在 Rspack 中集成 Rsdoctor 的原生插件，我们将 Rsdoctor 中耗时较大的数据处理逻辑下沉到 Rspack 的构建阶段。这一优化显著提升了 Rspack 中 Rsdoctor 的构建分析效率，减少了大约 **16%** 的分析时间，项目越大，提升效率越显著，[详情](/config/options/options#enablenativeplugin)。
...
在一个基于 Rspack 的 Arco 项目中，我们根据是否启用 **Rsdoctor Native Plugin** 进行了测试，得到了以下结果：

- Rsdoctor Native Plugin 对比测试结果（单位：秒）

|     | Build End (开启) | Open Report (开启) | Build End (未开启) | Open Report (未开启) |
| --- | ---------------- | ------------------ | ------------------ | -------------------- |
| 1   | 6.788            | 8.428              | 7.987              | 9.447                |
| 2   | 6.727            | 8.222              | 7.81               | 9.256                |
| 3   | 6.883            | 8.458              | 7.886              | 9.345                |
| 4   | 6.629            | 8.109              | 7.877              | 9.329                |
| 5   | 6.549            | 7.99               | 8.072              | 9.54                 |
