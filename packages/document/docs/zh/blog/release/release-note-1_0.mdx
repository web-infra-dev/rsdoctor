...
在经过一年的迭代和验证后，我们正式推出 **Rsdoctor 1.0** —— 一款专门为 [Rspack](https://rspack.dev/) 设计的构建分析工具，同时支持 [Rspack](https://rspack.dev/) 和 [webpack](https://webpack.js.org/) 的全链路构建分析，并且已经在社区及字节跳动内部的众多项目中得到充分验证和广泛应用。
...
然而，这些工具均存在一些不足，例如缺乏细粒度的构建细节展示、不具备全方位的构建问题分析能力、不兼容 Rspack、以及缺少构建预警扫描功能。
...
3. **Rspack 支持不足**：现有工具对 Rspack 的支持不够全面，尤其是不支持分析 [**Rspack 内置 Loader**](https://rspack.dev/guide/features/builtin-swc-loader) 。

### Rsdoctor 诞生

根据以上社区工具现状，我们决定为 Rspack 生态开发一个专注于构建产物及编译分析的工具 —— **Rsdoctor**。

我们为 Rsdoctor 设计了直观的用户界面，同时，Rsdoctor 通过结合多种工具的优点，扩展了 **Loader 打点能力**以更深入地分析编译阶段的 Loader 行为，**内置了产物及编译的扫描和检测规则**，并支持用户**自定义规则**。

它不仅支持 Rspack 和 webpack，也支持所有基于 Rspack 或 webpack 的工具和框架，例如：[Rsbuild](https://rsbuild.dev/)、[Rspress](https://rspress.dev/) 及 [Modern.js](https://modernjs.dev/) 等。
...
Rsdoctor 在 npm 上的周下载量已超过 **10 万次**。作为 Rspack 生态的构建分析工具，Rsdoctor 在字节跳动内部被开发人员广泛用于分析构建过程中的各类问题。
...
- 如果你在使用 Rsdoctor 0.4 或更早的版本，请留意 v1.0 包含少量不兼容更新：
