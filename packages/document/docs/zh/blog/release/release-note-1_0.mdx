...
Rsdoctor 不仅支持 Rspack 和 webpack，也支持所有基于 Rspack 或 webpack 的工具和框架，例如：[Rsbuild](https://rsbuild.dev/)、[Rspress](https://rspress.dev/) 及 [Modern.js](https://modernjs.dev/) 等。
...
- **新增 output.compressData 配置**：用于控制是否对分析数据进行压缩处理。默认情况下，该选项为 `true`，即会对分析数据进行压缩。
