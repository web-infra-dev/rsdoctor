...
经过一年的持续迭代与生产验证，我们正式发布 **Rsdoctor 1.0** —— 专为现代前端构建设计的深度分析工具。作为同时支持 [Rspack](https://rspack.dev/) 和 [webpack](https://webpack.js.org/) 的全链路观测工具，现已在多个大型项目中接入使用。
...
然而，这些工具仍然存在一些限制，例如无法提供更细粒度的构建细节展示和全方位的构建问题分析能力，对 Rspack 构建器不兼容，以及缺少对构建的预警扫描功能。
...
| 功能                | 描述                                                                          | 相关链接                                           |
| ------------------- | ----------------------------------------------------------------------------- | -------------------------------------------------- |
| Loader 编译行为分析 | 以目录与文件维度的 Loader 分析数据，展示单个文件编译行为                      | [Loaders 分析](/guide/usage/loaders-analysis)      |
...
在接下来的 6 ~ 12 个月时间里，Rsdoctor 将前置到 Rspack 底层，在现有 [Rspack Native Plugin](/config/options/options#enablenativeplugin) 及其他构建环节的基础上，支持更细粒度更精确的 Loader 分析。同时，将和 **AI 能力结合**，通过 **AI Chat Bot** 更智能更直观且更有针对性的帮助用户查找构建分析中的疑难杂症。
