...
<img src="/rsdoctor-1_0-banner.png" width="80%" style={{ margin: 'auto' }} />

在经过一年的不断迭代和验证后，我们正式推出 **Rsdoctor 1.0** —— 一款专门为 [Rspack](https://rspack.dev/) 而设计的构建分析工具，同时支持 [Rspack](https://rspack.dev/) 和 [webpack](https://webpack.js.org/) 的构建全链路分析工具，Rsdoctor 作为构建分析工具已经在社区及字节跳动内的众多项目中得到充分验证及使用。
...
总结来说，构建分析工具一直面临以下难点和痛点：

1. **细节不够深入**：传统工具**无法深入探索编译过程**的细节和变化，如果想要查看某个 Loader 编译细节，开发者常常需要手动设置断点逐步调试，效率低下。
2. **数据割裂与孤立**：现有的产物分析工具通常只提供产物分析数据展示，而不具备构建扫描规则能力，**无法主动为产物提供合理的优化建议，也不支持用户自定义规则**。
3. **Rspack 支持不足**：现有工具对 [Rspack](https://rspack.dev/) 的支持不够全面，尤其是不支持分析 [**Rspack 内置 Loader**](https://rspack.dev/guide/features/builtin-swc-loader) 。

### Rsdoctor 诞生

根据以上社区工具现状，我们决定为 [Rspack](https://rspack.dev/) 生态开发一个专注于构建产物及编译分析的工具 —— **Rsdoctor**。

我们为 Rsdoctor 设计了直观的用户界面，同时，Rsdoctor 通过结合多种工具的优点，扩展了 **Loader 打点能力**以更深入地分析编译阶段的 Loader 行为，**内置了产物及编译的扫描和检测规则**，并支持用户**自定义规则**。

它不仅支持 [Rspack](https://rspack.dev/) 和 [webpack](https://webpack.js.org/)，还支持基于 [Rspack](https://rspack.dev/) 和 [webpack](https://webpack.js.org/) 衍生的框架的构建分析，例如：[Rsbuild](https://rsbuild.dev/)、[Rspress](https://rspress.dev/) 及 [Modern.js](https://modernjs.dev/) 等。
...
<img
  ...
  src="/loader-timeline-overall.png"
/>
...
例如：通过 Rsdoctor 的构建时编译前后代码对比，发现经过 **babel-loader** 编译后，css 文件的 import 顺序变化，导致优先级最高的样式被默认样式配置覆盖，导致 css 样式无效。最终发现是 `babel-plugin-import` 的问题：[#606](https://github.com/umijs/babel-plugin-import/issues/606)。

<img style={{ width: '80%', margin: 'auto' }} src="/babel-import-error.png" />
...
<img style={{ width: '80%', margin: 'auto' }} src="/bundle-size-overall.png" />
...
<img style={{ width: '80%', margin: 'auto' }} src="/bundle-diff-assets.png" />

**后续 Rsdoctor 将计划支持 Bundle Diff 的主动防劣化能力**，例如：提供 Bundle Diff 的 GitHub Actions 以支持在 CI 阶段主动拦截产物不合理的增大。

### 5.「某个模块文件为什么会被打包进来？」

构建过程中，如果想要知道某个模块文件为什么会被打包到产物中，则可以在[产物分析](/guide/usage/bundle-size)页面中的 Modules 树图中点击该模块后面的 `Module Graph` 图标，查看该模块的上游依赖模块关系。

<img style={{ width: 600, margin: 'auto' }} src="/module-analysis-tree.png" />

**后续我们将计划结合 AI 能力，帮助用户更快的分析模块依赖链路问题。**

## 谁在使用

Rsdoctor 目前在 NPM 上周下载量已**超过 10+ 万次**。作为支持 **Rspack 及其衍生框架**（如 Rsbuild 等）的构建分析工具，Rsdoctor 在字节跳动内部应用于支持公司内开发同学进行构建分析。

在社区内，Rsdoctor 插件已被集成到 [Docusaurus](https://docusaurus.io/docs/api/plugins/@docusaurus/plugin-rsdoctor) 和 [Lynx](https://lynxjs.org/rspeedy/use-rsdoctor.html) 中。同时，Rsdoctor 也被 [Sentry](https://github.com/getsentry/sentry/pull/63865)、[NocoBase](https://github.com/nocobase/nocobase/blob/main/packages/core/build/src/buildPlugin.ts#L493) 和 [Grafana](https://github.com/grafana/grafana/pull/94729) 等项目所采用。
...
通过在 Rspack 中集成 Rsdoctor 的原生插件，我们将 Rsdoctor 中耗时较大的数据处理逻辑下沉到 Rspack 的构建阶段。这一优化显著提升了 Rspack 中 Rsdoctor 的构建分析效率，减少了整体分析时间，[详情](/config/options/options#enablenativeplugin)。
...
<img style={{ width: '80%', margin: 'auto' }} src="/search-modules.png" />
...
- 如果你正在使用 Rspack、Rsbuild、Rspress、Rslib 等基于 [Rspack](https://rspack.dev/) 和 [webpack](https://webpack.js.org/) 的框架，可以详见[快速开始](/guide/start/quick-start)来开始使用 Rsdoctor。
- 如果你正在使用 Rsdoctor 0.4 或更早的版本，请留意 1.0 版本包含一些不兼容更新，可以参照下方不兼容更新：
...
- 由于 Rspack 底层是 Rust 实现的，多线程特性限制了 Rsdoctor 对 Rspack 内置 **Loader** 的分析粒度。为此，我们计划在 [Rspack Native Plugin](/config/options/options#enablenativeplugin) 的基础上，深入 Rspack 内部进行 Loader 分析，提供更细致、更准确的 Loader 数据。
- Rsdoctor 的构建分析数据非常丰富，但用户可能刚开始不熟悉报告的使用。AI 能力可以理解用户的问题，从大量数据中提取合适的结果。**Rsdoctor 将与 AI 能力集成**，通过 AI 聊天机器人为用户提供更智能且有针对性的构建分析支持。
- Rsdoctor 会结合之前的 [Bundle Diff](/guide/usage/bundle-diff) 能力**在 CI 上拓展主动防劣化能力**，提供 [Rsdoctor CI Action](https://github.com/web-infra-dev/rsdoctor/discussions/491) 主动劣化监控能力。
