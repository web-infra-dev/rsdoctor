...
在经过一年的迭代与验证后，我们正式推出 **Rsdoctor 1.0** —— 一款为 [Rspack](https://rspack.dev/) 量身打造的构建分析工具，同时完全兼容 [webpack 5](https://webpack.js.org/)。Rsdoctor 提供全链路构建分析能力，已在字节跳动内部及众多社区项目中得到广泛应用与验证。
...
Rsdoctor 在 npm 上的周下载量已超过 **10 万次**。作为 Rspack 生态的构建分析工具，Rsdoctor 已在字节跳动内部得到广泛应用，帮助开发者高效分析和解决构建过程中的各类问题。
