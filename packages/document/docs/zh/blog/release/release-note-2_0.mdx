...
- **统一分析多 compiler 构建**：常规 Rspack 插件可以自动处理 multi-compiler、Rsbuild environments 和 child compiler，同时保持每个 compiler 的数据相互隔离。
- **更小的安装体积**：核心能力统一收敛到 `@rsdoctor/core`。在 `V2` 中，完整生产安装包含的 package 从 94 个减少到 41 个，体积比 1.6.2 减少 **53.6%**。
...
Rsdoctor 2.0 已移除 `@rsdoctor/mcp-server`，统一使用按需执行、无需维护服务连接的 Agent CLI。它不兼容 MCP 协议，也不是旧 API 的直接替代品；安装和迁移方式请参考 [AI](/guide/start/ai) 与 [2.0 迁移指南](/guide/migration/migration-v2#ai-工作流迁移)。

## 统一分析多 compiler 构建 {#one-report-for-every-compiler}
...
具体用法见[选择 compiler](/zh/guide/start/ai#selecting-a-compiler)。
...
| ------------------------------------------------- | -----------: | -----------: |
| 仓库顶层 package                                  |           13 |            6 |
| 顶层 package 的 `dependencies` 直接依赖声明数之和 |           88 |           36 |

这里统计的是源码中的直接依赖声明数，不是安装后的 package 数。依赖图还会包含根节点、传递依赖和 peer 依赖，因此节点数会更多。

### 更小的安装体积 {#smaller-production-install-footprint}

包结构和 runtime 依赖的精简，让 `@rsdoctor/core` 及其完整传递依赖的生产安装体积和 package 数量都减少了一半以上：

| 指标             | Rsdoctor V1.x | Rsdoctor V2 |    减少 |      降幅 |
| ---------------- | ------------: | ----------: | ------: | --------: |
| 第三方依赖包体积 |       25.8 MB |     12.0 MB | 13.8 MB | **53.6%** |
| 第三方依赖包数量 |            94 |          41 |      53 | **56.4%** |

Rspack 项目升级时需要使用 `@rsdoctor/core` 替换 `@rsdoctor/rspack-plugin`，具体步骤请参考 [2.0 迁移指南](/guide/migration/migration-v2#更新-rspack-插件)。
...
![Tree Graph 优化前](https://assets.rspack.rs/rsdoctor/tree-graph-before.gif)

**优化后**

![Tree Graph 优化后](https://assets.rspack.rs/rsdoctor/tree-graph-after.gif)
...
- **运行环境与构建工具**：要求使用 Node.js `^20.19.0 || >=22.12.0` 和 Rspack 2.0 及以上版本。Rspack 1.x 项目需要先升级 Rspack；webpack 项目应继续使用长期维护的 Rsdoctor 1.x，或先迁移到 Rspack。
...
完整示例请参考 [配置迁移说明](/guide/migration/migration-v2#configuration-migration)。
...
使用 CLI 时，请确保 `@rsdoctor/cli` 与 `@rsdoctor/core` 的版本一致。完整步骤请参考 [Rsdoctor 2.0 迁移指南](/guide/migration/migration-v2)。
