...
- Treemap 新增全屏模式、可收起侧边栏、面包屑导航和点击缩放。查看深层 Module 时，可以逐级下钻，同时保留当前所在层级的上下文。
- Asset 与 Chunk 筛选、Module 正则搜索、结果高亮定位，以及直接打开 Module 详情，缩短了从发现异常体积到确认具体代码的操作路径。
- Stat、Parsed 和 Gzipped 三种体积口径在 Treemap、侧边栏和 Tooltip 中保持一致；JavaScript Asset 筛选会隐藏与 Module 分析无关的资源，响应式布局和可展开卡片也能更充分地利用屏幕空间。
