...
- **lite**：启用 lite 模式。`features: { lite: true }` 和 `features: ['lite']` 均受支持。未显式配置 [`output.reportCodeType`](/config/options/output#reportcodetype) 时，lite 模式等同于将其设置为 `noAssetsAndModuleSource`：不包含模块源码和 Assets 代码，但保留模块打包后代码。lite 模式默认关闭。
