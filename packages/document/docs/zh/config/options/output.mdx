...
- **brief:** 在构建输出目录中生成 `rsdoctor-report.html`。所有构建分析数据都会嵌入这份独立文件，可以直接在浏览器中打开。更多配置请参阅 [mode: 'brief'](#mode-brief)。

:::tip
`output.mode` 不支持 `lite`。旧版 `mode: 'lite'` 配置已在 Rsdoctor 2.x 中废弃，仅为向后兼容而保留。请改用 [`output.reportCodeType`](#reportcodetype)。
:::
...
:::warning
旧版 `output.compressData` 配置已在 Rsdoctor 2.x 中废弃，仅为向后兼容而保留。请改用 `output.mode: 'brief'` 和 `output.options.type: ['json']`。参阅 [compressData 配置迁移](/config/options/options-v2#outputcompressdata)。
:::
