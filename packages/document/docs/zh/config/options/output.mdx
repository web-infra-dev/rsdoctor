...
- **类型：** `"noModuleSource" | "noAssetsAndModuleSource" | "noCode"`
- Rsdoctor 2.x 仅接受上面列出的字符串值。
- **可选：** `true`
...
new RsdoctorRspackPlugin({
  output: {
    reportCodeType: 'noModuleSource', // 或 'noAssetsAndModuleSource'
  }
}),
```
...
:::warning
已在 Rsdoctor 2.x 中移除且会被忽略。请使用 `output.mode: 'brief'`，并将 `output.options.type` 设置为 `['json']`。参考 [compressData 配置迁移](/guide/migration/migration-v2#outputcompressdata)。
:::
