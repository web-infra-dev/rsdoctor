...
这个问题通常是因为构建过程中存储源码信息，导致内存超限。请将 `output.reportCodeType` 设置为 `noCode` 或 `noAssetsAndModuleSource`，以降低内存占用。详情请参阅 [output.reportCodeType](/config/options/output#reportcodetype)。
