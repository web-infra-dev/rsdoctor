...
当我们**只需要**使用 Rsdoctor 的 [Bundle Size](../usage/bundle-size) 分析产物大小功能时，可以在接入 Rsdoctor 插件时配置对应的 [Features](/config/options/options) 配置项，参考下方代码：
...
Rsdoctor 提供的 Loader 耗时是**预估耗时**。为什么无法统计到准确耗时？这是因为 Loader 执行可能是**异步**函数也可能是**同步**函数，打包工具会**并行执行多个**不冲突的 Loader 函数，而 **JS 是单线程**的，多个 Loader 函数均可能**抢占当前的任务队列**，且 Loader 函数内的**异步逻辑无法识别**，导致单个 Loader 函数在执行过程中**可能横跨**多个其他 Loader 的执行过程，因此会存在如下图所示的三种情况：
...
因此，Rsdoctor 提供的 Loader 耗时是一个**预估**的数据，我们给出的耗时数据适配了上图中情况 1 和情况 2，对于情况 3 的解决方案，我们目前还在探索中。
...
可以在 `features` 数组中添加 `lite` 字段，使用 lite 模式。注意：因为 `features` 为数组时会覆盖掉默认配置，所以需要手动添加所需的功能：

- 如果需要构建时分析，在 `features` 数组中添加 `loader` 和 `plugins`，用于开启 `loader` 和 `plugins` 的耗时分析。
- 如果需要构建产物分析，在 `features` 数组中添加 `bundle`，用于开启构建产物分析。

下面的示例开启了 lite 模式以及构建时和构建产物分析：
...
- **原因**：构建过程中存储了源码信息，超过了内存限制，开启 `lite` 模式可以缓解此问题。
- **区别**：`lite` 模式和普通模式的区别在于不再存储**源码信息**，只存储**打包后的代码**，分析报告上的代码也将**只显示打包后的代码**。
...
如果只有 Source Size，没有 Bundled Size，或者 **Tree Map 中没有 Module 下钻**（即，没有 Modules 分块），则需要**手动开启 SourceMap**。**注意只在 Rsdoctor 开启时打开 SourceMap，不要影响线上产物**。
