...
编写自定义规则时，安装 @rsdoctor/core 并使用 @rsdoctor/core/rules 中的 defineRule 函数来定义统一的 Rsdoctor 规则。
...
- 组件代码 [Code](https://github.com/web-infra-dev/rsdoctor/blob/main/packages/client/src/components/Alerts/list.tsx)
...
- 组件代码 [Code](https://github.com/web-infra-dev/rsdoctor/blob/main/packages/client/src/components/Alert/view.tsx)
...
在进行规则检测分析时，对模块进行AST分析等操作是常见的。为了提供更多辅助功能，我们还提供了 `@rsdoctor/core/rule-utils`，其中包含了许多实用的工具函数和方法。
