...
编写自定义规则时，安装 `@rsdoctor/core` 并使用 `@rsdoctor/core` 中的 defineRule 函数来定义统一的 Rsdoctor 规则。
...
同时，Rsdoctor 会在警示列表中展示自定义规则的结果，详情参见下方[展示组件](#%E5%B1%95%E7%A4%BA%E7%BB%84%E4%BB%B6)。
...
核心规则仍使用此数据类型。当前客户端通过警示列表展示这类结果，不再提供专用的代码展示组件。
...
在进行规则检测分析时，对模块进行AST分析等操作是常见的。为了提供更多辅助功能，我们还提供了 `@rsdoctor/core`，其中包含了许多实用的工具函数和方法。
