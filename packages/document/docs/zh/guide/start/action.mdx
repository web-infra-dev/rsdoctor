...
- `target_branch`：非必需，目标分支名称，默认是 'main'。如果想用不固定的目标分支，即自动获取当前 MR 的 target branch，而不是固定的主分支，则可以使用下方配置：
    ...
    steps:
      ...
      - name: Bundle Analysis
        ...
        with:
          ...
          # 默认 'main'，如果想用不固定的目标分支，即自动获取当前 MR 的 target branch，则可以使用下方配置：
