...
Rsdoctor 提供了官方的 GitHub [rsdoctor-action](https://github.com/web-infra-dev/rsdoctor-action) 在 CI/CD 流程中轻松集成 Rsdoctor 分析功能。通过 GitHub Action，你可以在 Github 上每次构建时自动生成和对 Bundle 进行差异分析，进行 Bundle 劣化监控，持续优化项目性能。
...
**2. 需要使用 Brief 模式，同时开启 type 数组中添加 'json'，用于后续 GitHub Action 中上传分析数据。** 具体配置可查看 [output 配置](/config/options/output)。
...
- `on` 是指运行的时机，需要在 pull_request 和 push 时，pull_request 时 Rsdoctor Action 会拉取 baseline 和 current 进行 bundle diff，push 也就是 PR 合并时，会进行 baseline 更新上传的操作。
