...
对于同时包含 client、server 等多个 compiler 的构建，先列出报告中的 compiler：

```bash
rsdoctor-agent compilers list --data-file ./rsdoctor-data.json
```

JSON 结果中的 `data.compilers` 包含各 compiler 的 `name`、数据文件绝对路径 `dataFile` 和表示文件是否存在的 `available`。也可以通过 `query compilers_list` 获取列表。

使用列表中的准确 `name` 选择 compiler。直接分析命令和 `query` 都支持 `--compiler`：
...
`--compiler` 是 CLI 参数，放在 `--input` JSON 之外。CLI 根据报告的 compiler 索引解析数据文件相对路径，因此选择不同 compiler 时可以沿用同一个 `--data-file`。移动报告时，请一并复制索引中的 compiler 数据文件，并保持相对路径不变。

- 索引中只有一个 compiler 时，CLI 自动选择。
- 索引中有多个 compiler 时，必须指定 `--compiler`，即使输入文件本身包含某个 compiler 的数据。
- 没有 compiler 索引的旧报告仍可读取，但不要传入 `--compiler`。如需按名称选择，请使用支持输出索引的插件版本重新生成报告。

执行 `assets diff` 时，CLI 会在 baseline 和 current 报告中分别选择同名 compiler：
...
Compiler 选择错误以 JSON 输出到 stderr，并返回非零退出码。`COMPILER_REQUIRED` 和 `COMPILER_NOT_FOUND` 会在 `error.compilers` 中列出可选名称。如果某个 compiler 的 `available` 为 `false`，请恢复数据文件或重新生成报告后再分析。每次分析只读取一个 compiler，不会合并不同 compiler 的结果。
