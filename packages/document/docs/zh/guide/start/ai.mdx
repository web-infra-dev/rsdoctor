...
- 在项目中安装（可选，因为 Skill 可以自动通过 `npx` 调用 CLI）：
...
- Skill 执行示例：
  ...
  如上动图所示，AI 工具会依据 Rsdoctor Skill 执行相应的 `@rsdoctor/agent-cli` 命令，获取所需数据，并在整合分析后输出优化建议。同时，它还会提供下一步更深入的分析计划，你可以继续选择 `Next Step`，进一步研究项目构建问题。
