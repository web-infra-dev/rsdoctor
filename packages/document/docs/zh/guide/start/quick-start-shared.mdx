...
在 `next.config.ts` 的 [Rspack Config](https://rspack.dev/guide/tech/next) 中初始化 RsdoctorRspackPlugin 插件。
...
Next.js 在 `build` 执行结束后会终止终端服务，导致 Rsdoctor 在构建过程中运行的报告页面服务器关闭。为解决此问题，你可以使用 [@rsdoctor/cli](/guide/start/cli) 重新打开报告页面，无需重新执行构建操作。具体方法见第三步或本地执行 rsdoctor 命令：
