...
**Rsdoctor** 提供了 `Module 引用链分析` 模块，该模块主要是分析某个 Module 的依赖树，即被那些模块所依赖，即 [Rspack 的 stats.reasons](https://rspack.rs/config/stats#statsreasons)。
...
- **`Reasons`** : 顾名思义是 `[原因]` 的意思，即某个 `Module` 存在的原因。Reasons 就是该 `Module` 被哪些 `Module` 们引入，而整个 `Reasons Tree` 就是这个 `Module` 的整个上游引用链，包括了直接父级和间接父级们。[即 Rspack 的 stats.reasons。](https://rspack.rs/config/stats#statsreasons)
...
还可以使用 `rsdoctor-agent` 基于 Rsdoctor 生成的分析数据进一步排查，例如让 Agent 结合 `rsdoctor-data.json` 分析某个模块没有被 Tree shaking 的原因。使用方式请参考 [Agent CLI](/guide/start/ai#agent-cli)。
