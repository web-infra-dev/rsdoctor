...
还可以使用 `rsdoctor-agent` 基于 Rsdoctor 生成的分析数据进一步排查，例如让 Agent 结合 `rsdoctor-data.json` 分析某个模块没有被 Tree shaking 的原因。使用方式请参考 [Agent CLI](/zh/guide/start/ai#agent-cli)。
