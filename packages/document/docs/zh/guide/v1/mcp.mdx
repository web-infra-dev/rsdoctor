...
:::warning
`@rsdoctor/mcp-server` 已不再维护。Rsdoctor 2.x 的 AI 构建分析流程请改用 [`@rsdoctor/agent-cli`](/guide/start/ai#agent-cli)。

以下内容仅作为 Rsdoctor 1.x 的归档说明，其中使用旧版 `@rsdoctor/rspack-plugin` 包，不适用于 Rsdoctor 2.0。
:::
...
:::warning
此归档流程需要使用以下 Rsdoctor 1.x 插件版本：

- @rsdoctor/rspack-plugin >= 1.1.2

Rsdoctor 2.0 请改用 `@rsdoctor/agent-cli`。
:::

### 1. 插件配置

安装归档的 1.x Rspack 插件，然后在 Rspack 配置中添加 `RsdoctorRspackPlugin`：
...
- 检查 Rsdoctor 插件版本是否符合要求。[版本要求](#-版本要求)
