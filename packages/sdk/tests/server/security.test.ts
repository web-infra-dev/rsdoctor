...
describe('test server/security.ts', () => {
  ...
  it('allows local request hosts', () => {
    ...
    expect(isAllowedRequestHost('127.1.2.3:3000')).toBe(true);
  ...
  it('rejects non-local request hosts', () => {
    ...
    expect(isAllowedRequestHost('192.168.1.10:3000')).toBe(false);
    expect(isAllowedRequestHost('10.0.0.5:3000')).toBe(false);
    expect(isAllowedRequestHost('8.8.8.8:3000')).toBe(false);
